    ...
    <div class="col-12">
        <div class="admin-card">
            <div class="admin-card-header">
                <h4 class="card-title">User WhatsApp Request Report</h4>
            </div>
            <form id="searchForm">
                <div class="row">
                    <div class="col-md-12">
                    ...
                    <table v-once id="manage-user-report" class="display nowrap" style="width: 100%">
                        <thead>
                            <tr>
                                ...
                                <th>Station ID</th>
                                <th>Name</th>
                                <th>WhatsApp Request No.</th>
                                <th>Message</th>
                                <th>Compony WhatsApp No</th>
                                <th>Status</th>
                                <th>Action</th>
                            </tr>
                        </thead>
...
$(document).ready(function() {
    ...
    var columns = [
        ...
        {
            data: "user_whatsapp_no"
        },
        {
            data: "message"
        },
        {
            data: "admin_whatsapp_no"
        },
        {
        ...
        { data: 'status',
          render: function (data, type, row) {
              if (row.status === "Pending") {
                  return `
                      <label class="btn btn-success waves-effect btn-sm" onclick="approve(${row.id}, 1)">
                          Approve
                      </label>
                      <label class="btn btn-danger waves-effect btn-sm" onclick="reject(${row.id}, 2)">
                          Reject
                      </label>`;
              } else {
                  return "";
    ...
    var table = $("#manage-user-report").DataTable({
        ...
        ajax: {
            url: "{{ url('1Rto5efWp86Z/get-user-whatsapp-request') }}",
            type: "POST",
            headers: {
    ...
    function approve(id, status) {
        Swal.fire({
            ...
            if (result.value) {
                ...
                $.ajax({
                    type: "POST",
                    url: "{{url('/1Rto5efWp86Z/approve-whatsapp-notification')}}",
                    data: {
                        id: id,
                        status: status,
                    },
    ...
    $.ajax({
        ...
        success: function (resp) {
            if (resp.code === 200) {
                ...
                fileLink.setAttribute("download", "WhatsAppNotificationReport.xls");
