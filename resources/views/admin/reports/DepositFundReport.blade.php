...
<script>
    $(document).ready(function() {
        ...
        var reportsTable = $("#direct-income-report").DataTable({
            ...
            ajax: {
                url: '{{ url('1Rto5efWp86Z/getconfirmaddrtrans') }}',
                type: "POST",
                data: function(d) {
                    i = 0;
                    i = d.start + 1;
                ...
                dataSrc: function(json) {
                    if (json.code === 200) {
                        ...
                        $('#pending').text('Pending:' + json.data.pending);
                        $('#confirmed').text('Confirmed:' + json.data.confirmed);
                        $('#expired').text('Expired:' + json.data.expired);
                        $('#confirmedsum').text('Total Confirmed Amount:' + json.data.confirmedsum);
                        $('#expiredsum').text('Total Expired Amount:' + json.data.expiredsum);
                        $('#totalsum').text('Total Recived Amount:' + json.data.topup_bal);
                        ...
                        $('#pending').text('Pending:' + '0');
                        $('#confirmed').text('Confirmed:' + '0');
                        $('#expired').text('Expired:' + '0');
                        $('#confirmedsum').text('Total Confirmed Amount:' + '0');
                        $('#expiredsum').text('Total Expired Amount:' + '0');
                        $('#totalsum').text('Total Recived Amount:' + '0');
            ...
            columns: [{
                ...
                },
                {
                    data: "status_url",
                    render: function(data) {
                        return "<a href='" + data + "' target='_blank'>Checkout</a>";
                    },
                },
                {
                    data: "hash_unit"
